...
exports.postUserInfo = (req, res, next) => {
    let loadedUser;
    Registeration.findOne({ emailAddress: req.body.emailAddress?.toLowerCase() }, async (err, doc) => {

        if (!err) {
            if (doc) {
                loadedUser = doc;
                let isAuthenticated = await bcrypt.compare(req.body.password, doc.password);
                if (isAuthenticated) {
                    req.session.user = loadedUser;
                    // res.cookie('email', `${loadedUser.emailAddress}`, { httpOnly: true });
                    req.session.cookie.httpOnly = true;
                    res.send({ status: "success", statusCode: 200, message: `Welcome ${doc.fullName}` });
                } else {
                    res.status(401).send({ status: "Failed", statusCode: 401, message: "Wrong password." });
                }
            } else {
                res.status(208).send({
                    status: "Failed",
...
exports.postLogout = (req, res, next) => {
    req.session.destroy(() => {
        res.send({ status: "success", statusCode: 200, message: "User logged out!" });
